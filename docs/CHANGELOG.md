...
- **Fixed multiple Reviewer issues**: (1) Removed dead `--glow-hue` CSS variable from `.tile-sprout.fresh-bloom` in styles.css — it was declared but never consumed by any property. (2) Removed dead `revealHistoryToggle` export from garden-history.js that was no longer imported by script.js. (3) Applied `pruned-tile` CSS class during tile restoration in persistence.js so restored pruned tiles retain their visual accent across sessions. (4) Added a maximum poll count (60 polls × 500ms = 30s) to the `statsRevealedObserver` setInterval in script.js to prevent an indefinite interval leak when stats are never revealed. (5) Applied `fresh-bloom` class to restored grown tiles so the introductory bloom glow animation plays on garden load. (6) Consolidated 7 scattered `prefers-reduced-motion` media query blocks into a single comprehensive block at the top of the file, reducing 30 `!important` flags across 7 blocks to a single maintainable location.

- **Added fresh-bloom life force glow animation**: When a tile first reaches its `grown` state, a soft radial ambient glow now blooms behind the sprout using a CSS `::after` pseudo-element with `box-shadow` and opacity animation — GPU-friendly, no layout thrashing. The glow starts bright and gently fades over ~8 seconds (matching `CYCLE_HOLD_BLOOM`), giving each freshly-bloomed flower a visible "life force" that distinguishes it from wilting tiles. The glow color is tinted with the tile's own primary petal color (set via `--bloom-color` CSS variable from `applyTileColors()`), making each bloom feel unique. A `fresh-bloom` class is added on the `.tile-sprout` when transitioning to `grown` and removed when wilting begins, in both `plantTile()` (first cycle) and `startGrowthCycle()` (perennial regrowth). Fully respects `prefers-reduced-motion`.
