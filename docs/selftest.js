...
export async function checks() {
  ...
  if (!groundDarkeningGardenState) {
    ...
    // Verify the ground darkening is actually being applied by inspecting
    // the real ground material colour relative to the base seasonal colour.
    // During Light Drizzle, each channel must be <= 0.99 of the base (darkened).
    // During Clear, the ground must not be artificially dark.
    if (groundDarkeningGardenState && groundDarkeningGardenState.groundMat &&
        groundDarkeningGardenState.baseGroundColor &&
        groundDarkeningGardenState.weather) {
      const phase = groundDarkeningGardenState.weather.getPhase();
      const factor = typeof groundDarkeningGardenState.weather.getGroundDarkeningFactor === 'function'
        ? groundDarkeningGardenState.weather.getGroundDarkeningFactor()
      ...
      if (phase === 'Light Drizzle' && factor > 0.1) {
        // The darkening formula is: darkenMul = 1 - 0.15 * factor
        // With factor > 0.1, darkenMul < 0.985, so each channel should be
        // at most 0.99 of the base (allowing small tolerance).
        const maxExpectedR = base.r * 0.99;
        const maxExpectedG = base.g * 0.99;
        const maxExpectedB = base.b * 0.99;
        if (ground.r > maxExpectedR && ground.g > maxExpectedG && ground.b > maxExpectedB) {
          problems.push('During Light Drizzle with factor ' + factor.toFixed(3) +
            ', ground colour ' + ground.getHexString() + ' is not darker than base ' +
            base.getHexString() + ' — expected each channel <= 0.99 of base (rain darkening not applying).');
        }
      } else if (phase === 'Clear' && factor < 0.01) {
        // During Clear, rain darkening is fully off (factor ≈ 0).
        // The ground should be at or near the base seasonal colour, but
        // the "darkest-of-three" rule can make it slightly darker during
        ...
        const _winterGround = new THREE.Color(0x3a2a1a);
        ...
        const eps = 0.012;
        if (ground.r < expectedMin.r - eps ||
            ground.g < expectedMin.g - eps ||
            ground.b < expectedMin.b - eps) {
          problems.push('During Clear weather (factor=' + factor.toFixed(3) +
            ', winterLegacyBlend=' + winterBlend.toFixed(3) + '), ground colour ' +
            ground.getHexString() + ' is darker than expected relative to base ' +
            base.getHexString() + ' — expected at least ~' + expectedMin.getHexString() +
            ' (base lerped by winterLegacyBlend=' + winterBlend.toFixed(3) + ').');
        }
      }
    }
  ...
  if (!plant1 || !plant1.leafMat) {
    ...
    // Verify the weather module's interpolation produces the expected
    // divergence between Clear and Light Drizzle leaf wetness values.
    // Reimplement the interpolation logic locally to test the PHASES config.
    // If the PHASES values or the interpolation formula changes, this will fail.
    const PHASES_TEST = [
    ...
    function interpolateLeaf(t) {
      for (let i = 0; i < PHASES_TEST.length - 1; i++) {
        const a = PHASES_TEST[i];
        const b = PHASES_TEST[i + 1];
        if (t >= a.t && t <= b.t) {
          const seg = a.t === b.t ? 0 : (t - a.t) / (b.t - a.t);
          const eased = seg * seg * (3 - 2 * seg);
          return {
      ...
      const last = PHASES_TEST[PHASES_TEST.length - 1];
      return { roughness: last.leafRoughness, metalness: last.leafMetalness };
    }

    // Sample at t=0.1 (well within Clear) and t=0.75 (well within Light Drizzle)
    const clearVals = interpolateLeaf(0.1);
    const drizzleVals = interpolateLeaf(0.75);

    const roughDelta = clearVals.roughness - drizzleVals.roughness;
    const metalDelta = drizzleVals.metalness - clearVals.metalness;

    if (roughDelta < 0.2) {
      problems.push('Leaf wetness roughness delta between Clear and Light Drizzle is ' +
        roughDelta.toFixed(3) + ' (clear=' + clearVals.roughness.toFixed(3) +
        ', drizzle=' + drizzleVals.roughness.toFixed(3) +
        ') — expected drop of at least 0.2 for a visible sheen.');
    }
    if (metalDelta < 0.01) {
      problems.push('Leaf wetness metalness delta between Clear and Light Drizzle is ' +
        metalDelta.toFixed(3) + ' (clear=' + clearVals.metalness.toFixed(3) +
        ', drizzle=' + drizzleVals.metalness.toFixed(3) +
        ') — expected increase of at least 0.01.');
    }
    // Verify the wet metalness doesn't exceed 0.1
    if (drizzleVals.metalness > 0.1) {
      problems.push('Leaf wetness metalness during Light Drizzle would be ' +
        drizzleVals.metalness.toFixed(3) + ', expected <= 0.05 for a subtle, non-glossy effect.');
    }
    ...
    try {
      ...
      Object.keys(expectedKeywords).forEach(function(phase) {
        ...
        if (missingKeywords.length > 0) {
          problems.push('weatherContexts["' + phase + '"] first variant = "' + ctx + '" — missing acceptance keywords: ' + missingKeywords.join(', ') + '.');
        }
        // Verify variants are unique within the phase
        for (var vi2 = 1; vi2 < arr.length; vi2++) {
          if (arr[vi2] === arr[0]) {
            problems.push('weatherContexts["' + phase + '"] variant ' + vi2 + ' is identical to variant 0 — each variant must be unique (issue #543).');
          }
          for (var vj2 = vi2 + 1; vj2 < arr.length; vj2++) {
            if (arr[vi2] === arr[vj2]) {
              problems.push('weatherContexts["' + phase + '"] variant ' + vi2 + ' equals variant ' + vj2 + ' — each variant must be unique (issue #543).');
            }
          }
        }
      });
      ...
      // --- Test 6: weatherContextFor returns correct strings (first variant) ---
      Object.keys(expectedKeywords).forEach(function(phase) {
        var ctx = gardenState.weatherContextFor(phase);
        if (ctx !== contexts[phase][0]) {
          problems.push('weatherContextFor("' + phase + '") returned "' + ctx + '", expected "' + contexts[phase][0] + '" (first variant).');
        }
      });
      ...
      // --- Test 7: Every phase gets a unique first variant ---
      var seenContexts = {};
      phaseOrder.forEach(function(phase) {
        var arr = contexts[phase];
        if (!arr || arr.length === 0) {
        ...
        if (!ctx || ctx.trim().length === 0) {
          problems.push('weatherContexts["' + phase + '"] first variant is empty.');
          return;
        }
        if (seenContexts[ctx]) {
          problems.push('weatherContexts["' + phase + '"] first variant shares the same string as "' + seenContexts[ctx] + '" — each phase must have a unique first variant.');
        }
  ...
  if (typeof gardenState.updateTimeOfDayDescription !== 'function') {
    ...
    try {
      ...
      Object.keys(expectedTimeKeywords).forEach(function(phase) {
        ...
        if (missingKeywords.length > 0) {
          problems.push('timeContexts["' + phase + '"] first variant = "' + ctx + '" — missing acceptance keywords: ' + missingKeywords.join(', ') + '.');
        }
        // Verify variants are unique within the phase
        for (var vi2 = 1; vi2 < arr.length; vi2++) {
          if (arr[vi2] === arr[0]) {
            problems.push('timeContexts["' + phase + '"] variant ' + vi2 + ' is identical to variant 0 — each variant must be unique (issue #543).');
          }
          for (var vj2 = vi2 + 1; vj2 < arr.length; vj2++) {
            if (arr[vi2] === arr[vj2]) {
              problems.push('timeContexts["' + phase + '"] variant ' + vi2 + ' equals variant ' + vj2 + ' — each variant must be unique (issue #543).');
            }
          }
        }
      });
      ...
      // --- Test 7: timeContextFor returns correct strings (first variant) ---
      Object.keys(expectedTimeKeywords).forEach(function(phase) {
        var ctx = gardenState.timeContextFor(phase);
        if (ctx !== timeContexts[phase][0]) {
          problems.push('timeContextFor("' + phase + '") returned "' + ctx + '", expected "' + timeContexts[phase][0] + '" (first variant).');
        }
      });
      ...
      // --- Test 8: Every phase gets a unique first variant ---
      ...
      timePhaseOrder.forEach(function(phase) {
        ...
        var ctx = arr[0];
        if (!ctx || ctx.trim().length === 0) {
          problems.push('timeContexts["' + phase + '"] first variant is empty.');
          return;
        }
        if (seenTimeContexts[ctx]) {
          problems.push('timeContexts["' + phase + '"] first variant shares the same string as "' + seenTimeContexts[ctx] + '" — each phase must have a unique first variant.');
        }
  ...
  if (typeof gardenState.updateSeasonDescription !== 'function') {
    ...
    try {
      ...
      Object.keys(expectedSeasonKeywords).forEach(function(phase) {
        ...
        if (missingKeywords.length > 0) {
          problems.push('seasonContexts["' + phase + '"] first variant = "' + ctx + '" — missing acceptance keywords: ' + missingKeywords.join(', ') + '.');
        }
        // Verify variants are unique within the phase
        for (var vi2 = 1; vi2 < arr.length; vi2++) {
          if (arr[vi2] === arr[0]) {
            problems.push('seasonContexts["' + phase + '"] variant ' + vi2 + ' is identical to variant 0 — each variant must be unique (issue #543).');
          }
          for (var vj2 = vi2 + 1; vj2 < arr.length; vj2++) {
            if (arr[vi2] === arr[vj2]) {
              problems.push('seasonContexts["' + phase + '"] variant ' + vi2 + ' equals variant ' + vj2 + ' — each variant must be unique (issue #543).');
            }
          }
        }
      });
      ...
      // --- Test 7: seasonContextFor returns correct strings (first variant) ---
      Object.keys(expectedSeasonKeywords).forEach(function(phase) {
        var ctx = gardenState.seasonContextFor(phase);
        if (ctx !== seasonContexts[phase][0]) {
          problems.push('seasonContextFor("' + phase + '") returned "' + ctx + '", expected "' + seasonContexts[phase][0] + '" (first variant).');
        }
      });
      ...
      phaseOrder.forEach(function(phase) {
        ...
        var ctx = arr[0];
        if (!ctx || ctx.trim().length === 0) {
          problems.push('seasonContexts["' + phase + '"] first variant is empty.');
          return;
        }
        if (seenSeasonContexts[ctx]) {
          problems.push('seasonContexts["' + phase + '"] first variant shares the same string as "' + seenSeasonContexts[ctx] + '" — each phase must have a unique first variant.');
        }
  ...
  /* ---------- Variant cycling checks (issue #543) ---------- */
  // Verify that _variantState is exposed, has the correct structure,
  // and that timed variant rotation works correctly.
  if (!gardenState || !gardenState._variantState) {
    problems.push('window.__gardenState._variantState is not set — variant cycling state not exposed (issue #543).');
  } else {
    const vs = gardenState._variantState;
    const requiredKeys = ['timeOfDay', 'weather', 'season'];
    const stateKeys = ['lastPhase', 'lastChangeTime', 'variantIndex'];

    requiredKeys.forEach(function(key) {
      if (!vs[key] || typeof vs[key] !== 'object') {
        problems.push('_variantState["' + key + '"] is missing or not an object.');
        return;
      }
      stateKeys.forEach(function(sk) {
        if (vs[key][sk] === undefined) {
          problems.push('_variantState["' + key + '"].' + sk + ' is undefined — required variant tracking property missing.');
        }
      });
    });

    // Verify VARIANT_CYCLE_MS is 30000
    if (gardenState.VARIANT_CYCLE_MS !== 30000) {
    ...
    ['Morning', 'Midday', 'Evening', 'Night'].forEach(function(phase) {
      ...
      if (arr && arr.length >= 5) {
        for (var ti = 0; ti < arr.length; ti++) {
          ...
          for (var tj = ti + 1; tj < arr.length; tj++) {
            if (arr[ti] === arr[tj]) {
              problems.push('timeContexts["' + phase + '"] variant ' + ti + ' equals variant ' + tj + ' — each variant must be unique (issue #543).');
            }
          }
        }
      }
    ...
    ['Clear', 'Overcast', 'Light Drizzle'].forEach(function(phase) {
      ...
      if (arr && arr.length >= 5) {
        for (var ti = 0; ti < arr.length; ti++) {
          ...
          for (var tj = ti + 1; tj < arr.length; tj++) {
            if (arr[ti] === arr[tj]) {
              problems.push('weatherContexts["' + phase + '"] variant ' + ti + ' equals variant ' + tj + ' — each variant must be unique (issue #543).');
            }
          }
        }
      }
    ...
    ['Spring', 'Summer', 'Autumn', 'Winter'].forEach(function(phase) {
      ...
      if (arr && arr.length >= 5) {
        for (var ti = 0; ti < arr.length; ti++) {
          ...
          for (var tj = ti + 1; tj < arr.length; tj++) {
            if (arr[ti] === arr[tj]) {
              problems.push('seasonContexts["' + phase + '"] variant ' + ti + ' equals variant ' + tj + ' — each variant must be unique (issue #543).');
            }
          }
        }
      }
    ...
    // Test timed cycling: set variant state timestamps to simulate 35s of stability,
    // then call updateTimeOfDayDescription and verify variant 1 is used.
    var origSeasonText = document.getElementById('season-display') ? document.getElementById('season-display').textContent : '';
    var origTimeText = document.getElementById('time-display') ? document.getElementById('time-display').textContent : '';
    var origWeatherText = document.getElementById('weather-display') ? document.getElementById('weather-display').textContent : '';

    try {
      document.getElementById('time-display').textContent = 'Morning';
      document.getElementById('weather-display').textContent = 'Clear';
      document.getElementById('season-display').textContent = 'Summer';

      var vsTime = vs.timeOfDay;
      vsTime.lastPhase = 'Morning';
      vsTime.variantIndex = 0;
      vsTime.lastChangeTime = performance.now() - 35000; // 35s ago, past the 30s threshold

      var plotEl = document.getElementById('plot-description');
      var growingEl = document.getElementById('growing-description');
      var origPlotText = plotEl.textContent;
      var origGrowingText = growingEl.textContent;

      plotEl.textContent = 'A young seedling rises.';
      growingEl.textContent = 'A young seedling rises.';

      gardenState.updateTimeOfDayDescription();

      var morningVariants = gardenState.timeContexts['Morning'];
      var expectedVariant1 = morningVariants[1];
      var plotText = plotEl.textContent;

      if (plotText.indexOf(expectedVariant1) === -1) {
        var expectedVariant0 = morningVariants[0];
        if (plotText.indexOf(expectedVariant0) !== -1) {
          problems.push('Variant cycling test: after 35s stable Morning, timeOfDay context used variant 0 instead of variant 1 — cycling did not advance (issue #543).');
        } else if (plotText.indexOf(morningVariants[0]) === -1 && plotText.indexOf(morningVariants[1]) === -1) {
          problems.push('Variant cycling test: plot text "' + plotText + '" does not contain any Morning variant (issue #543).');
        }
      }

      // Restore original descriptions
      plotEl.textContent = origPlotText;
      growingEl.textContent = origGrowingText;

      // Reset variant state to prevent real cycling from kicking in
      vsTime.lastChangeTime = performance.now();
      vsTime.lastPhase = 'Morning';
      vsTime.variantIndex = 0;
      vs.weather.lastChangeTime = performance.now();
      vs.weather.lastPhase = 'Clear';
      vs.weather.variantIndex = 0;
      vs.season.lastChangeTime = performance.now();
      vs.season.lastPhase = 'Summer';
      vs.season.variantIndex = 0;

    } finally {
      // Restore original DOM displays
      if (document.getElementById('time-display')) {
        document.getElementById('time-display').textContent = origTimeText;
      }
      if (document.getElementById('weather-display')) {
        document.getElementById('weather-display').textContent = origWeatherText;
      }
      if (document.getElementById('season-display')) {
        document.getElementById('season-display').textContent = origSeasonText;
      }
      // Re-run updates with restored state
      if (typeof gardenState.updateSeasonDescription === 'function') {
        gardenState.updateSeasonDescription();
      }
      if (typeof gardenState.updateTimeOfDayDescription === 'function') {
  ...
  // Established sprouts (leafGen >= 1) should survive autumn as bare stems
  // First-generation sprouts (leafGen < 1) should be removed in autumn
  // Wintered stems should be persisted and restored
  // DOM should mention dormant stems during winter
  {
    const gs = window.__gardenState;
    if (!gs) {
      problems.push('window.__gardenState is not set — cannot check wintered stems (issue #652).');
    } else {
      const groundSeeds = gs.groundSeeds;
      if (groundSeeds) {
        // Check winteredStems structure if present
        if (groundSeeds.winteredStems && groundSeeds.winteredStems.length > 0) {
          groundSeeds.winteredStems.forEach(function(ws, i) {
            if (!ws.group) {
              problems.push('groundSeeds.winteredStems[' + i + '].group is missing (issue #652).');
            }
            if (!ws.stem) {
              problems.push('groundSeeds.winteredStems[' + i + '].stem is missing (issue #652).');
            }
            if (!ws.basePos || typeof ws.basePos.x !== 'number') {
              problems.push('groundSeeds.winteredStems[' + i + '].basePos is missing or invalid (issue #652).');
            }
            if (typeof ws.leafGenCount !== 'number' || ws.leafGenCount < 1) {
              problems.push('groundSeeds.winteredStems[' + i + '].leafGenCount is ' + ws.leafGenCount + ', expected >= 1 (issue #652).');
            }
            // Verify wintered stem has no leaves (bare stem)
            if (ws.group && ws.group.children) {
              // The group may have leaves being re-grown in early spring, but in winter it should be bare
              // Check children - in winter the stem should be the only child
            }
          });
        }

        // Check that the sprouts array only contains established sprouts during/after autumn
        if (groundSeeds.sprouts && groundSeeds.sprouts.length > 0) {
          const allEstablished = groundSeeds.sprouts.every(function(sp) {
            return sp.leafGenCount >= 1;
          });
          // During autumn late phase, non-established sprouts should be removed.
          // We can check if winteredStems also exists, which would confirm the split happened.
          if (groundSeeds.winteredStems && groundSeeds.winteredStems.length > 0) {
            // Only established sprouts should remain
            const nonEstablished = groundSeeds.sprouts.filter(function(sp) {
              return sp.leafGenCount < 1;
            });
            if (nonEstablished.length > 0) {
              problems.push('Autumn removal incomplete: ' + nonEstablished.length + ' first-generation sprouts remain in groundSeeds.sprouts when winteredStems exist — only leafGen >= 1 should persist (issue #652).');
            }
          }
        }

        // Check DOM for winter stem references during winter
        const seasonDisplay = document.getElementById('season-display');
        if (seasonDisplay && seasonDisplay.textContent.trim() === 'Winter') {
          // During winter, the growing/plot description should mention dormant stems
          // if winteredStems exist
          if (groundSeeds.winteredStems && groundSeeds.winteredStems.length > 0) {
            const growingDesc = document.getElementById('growing-description');
            const plotDesc = document.getElementById('plot-description');
            if (growingDesc && growingDesc.textContent.indexOf('bare stems') === -1 &&
                growingDesc.textContent.indexOf('Dormant') === -1) {
              problems.push('During winter, growing-description should mention dormant/bare stems when winteredStems exist, got: "' + growingDesc.textContent + '" (issue #652).');
            }
            if (plotDesc && plotDesc.textContent.indexOf('bare stems') === -1 &&
                plotDesc.textContent.indexOf('Dormant') === -1) {
              problems.push('During winter, plot-description should mention dormant/bare stems when winteredStems exist, got: "' + plotDesc.textContent + '" (issue #652).');
            }
          }
        }

        // Check DOM for returning shoots mention during spring
        if (seasonDisplay && seasonDisplay.textContent.trim() === 'Spring') {
          if (groundSeeds.sprouts && groundSeeds.sprouts.length > 0) {
            const anyRegrown = groundSeeds.sprouts.some(function(sp) {
              return sp.leafGenCount >= 1 && sp.leaves && sp.leaves.length > 0;
            });
            // This check is soft — only flag if there's evidence of regrowth without DOM update
          }
        }
      }
    }

    // Persistence round-trip for wintered stems
    if (window.__gardenState) {
      const realWs = (window.__gardenState.groundSeeds && window.__gardenState.groundSeeds.winteredStems) ?
        window.__gardenState.groundSeeds.winteredStems.slice() : null;
      try {
        // Inject a winteredStems state into groundSeeds for save/load testing
        if (!window.__gardenState.groundSeeds) {
          window.__gardenState.groundSeeds = {};
        }
        window.__gardenState.groundSeeds.winteredStems = [
          {
            group: new THREE.Group(),
            stem: new THREE.Mesh(new THREE.BoxGeometry(0.001, 0.001, 0.001)),
            basePos: { x: 0.1, y: 0.005, z: 0.2 },
            leafGenCount: 1
          }
        ];
        // Save the state
        ...
        if (!loaded652) {
          problems.push('saveGardenState + loadGardenState round-trip returned null — cannot verify winteredStems persistence (issue #652).');
        } else if (!loaded652.winteredStems) {
          problems.push('winteredStems was saved but not present in loaded state — persistence missing (issue #652).');
        } else {
          if (!loaded652.winteredStems.positions || loaded652.winteredStems.positions.length !== 1) {
            problems.push('winteredStems.positions should have 1 entry after round-trip, got ' +
              (loaded652.winteredStems.positions ? loaded652.winteredStems.positions.length : 0) + ' (issue #652).');
          } else {
            const pos = loaded652.winteredStems.positions[0];
            if (Math.abs(pos.x - 0.1) > 0.001 || Math.abs(pos.z - 0.2) > 0.001) {
              problems.push('winteredStems position round-trip: expected (0.1, 0.2), got (' + pos.x + ', ' + pos.z + ') (issue #652).');
            }
          }
          if (!loaded652.winteredStems.leafGenCounts || loaded652.winteredStems.leafGenCounts.length !== 1) {
            problems.push('winteredStems.leafGenCounts should have 1 entry after round-trip, got ' +
              (loaded652.winteredStems.leafGenCounts ? loaded652.winteredStems.leafGenCounts.length : 0) + ' (issue #652).');
          } else if (loaded652.winteredStems.leafGenCounts[0] !== 1) {
            problems.push('winteredStems leafGenCount round-trip: expected 1, got ' + loaded652.winteredStems.leafGenCounts[0] + ' (issue #652).');
          }
        }

        // Test fast-forward preserves winteredStems when appropriate
        const ff652 = fastForwardState({
          seasonProgress: 0.75, // ~Autumn
          dayNightProgress: 0.5,
          weatherProgress: 0.5,
          plant1Maturity: 1,
          firstPlantGrown: true,
          winteredStems: {
            positions: [{ x: 0.1, y: 0.005, z: 0.2 }],
            leafGenCounts: [1]
          },
          timestamp: Date.now() - 1000 // 1 second ago
        });
        if (!ff652) {
          problems.push('fastForwardState returned null when testing winteredStems preservation (issue #652).');
        } else if (!ff652.winteredStems) {
          // After only 1s, the season has barely moved from Autumn (0.75).
          // We should still be in autumn or early winter, so winteredStems should persist.
          problems.push('fastForwardState lost winteredStems after 1s: seasonProgress=' +
            (ff652.seasonProgress ? ff652.seasonProgress.toFixed(3) : 'null') + ' (issue #652).');
        }

        // Clear test state
        clearGardenState();
      } finally {
        if (window.__gardenState && window.__gardenState.groundSeeds) {
          window.__gardenState.groundSeeds.winteredStems = realWs;
        }
      }
    }
  }

  /* ---------- Wintered stems DOM and lifecycle checks (issue #652) ---------- */
  // Verify that when wintered stems exist in groundSeeds, the DOM description
  // text includes a reference to dormant stems or bare stems (during winter).
  const gs652 = window.__gardenState;
  if (gs652 && gs652.groundSeeds && gs652.groundSeeds.winteredStems && gs652.groundSeeds.winteredStems.length > 0) {
    const seasonEl = document.getElementById('season-display');
    const currentSeason = seasonEl ? seasonEl.textContent.trim() : '';

    // If currently in Winter, check the plot/growing description mentions dormant stems
    if (currentSeason === 'Winter') {
      const growingDesc = document.getElementById('growing-description');
      const plotDesc = document.getElementById('plot-description');
      if (growingDesc && growingDesc.textContent.indexOf('Dormant') === -1 &&
          growingDesc.textContent.indexOf('dormant') === -1 &&
          growingDesc.textContent.indexOf('bare stems') === -1) {
        problems.push('Wintered stems exist but growing-description does not mention "dormant" or "bare stems" (issue #652). Got: "' + growingDesc.textContent + '"');
      }
      if (plotDesc && plotDesc.textContent.indexOf('Dormant') === -1 &&
          plotDesc.textContent.indexOf('dormant') === -1 &&
          plotDesc.textContent.indexOf('bare stems') === -1) {
        problems.push('Wintered stems exist but plot-description does not mention "dormant" or "bare stems" (issue #652). Got: "' + plotDesc.textContent + '"');
      }
    }

    // Verify each wintered stem has a leafGenCount >= 1
    for (let wi = 0; wi < gs652.groundSeeds.winteredStems.length; wi++) {
      const ws = gs652.groundSeeds.winteredStems[wi];
      if (typeof ws.leafGenCount !== 'number' || ws.leafGenCount < 1) {
        problems.push('Wintered stem #' + wi + ' has leafGenCount=' + ws.leafGenCount + ', expected >= 1 (issue #652).');
      }
      if (ws.stem && ws.stem.material) {
        // During Winter, stems should be desaturated (brown) with reduced opacity
        const col = ws.stem.material.color;
        if (col) {
          // Should be a brownish hue (R > G and R > B roughly)
          const isBrownish = (col.r > col.g * 0.9 && col.r > col.b * 0.9 && col.g > col.b * 0.7);
          if (!isBrownish) {
            problems.push('Wintered stem #' + wi + ' color (' + col.getHexString() + ') is not desaturated brown (issue #652).');
          }
        }
      }
    }

    // Check that wintered stems have no leaves attached (they were shed)
    for (let wi = 0; wi < gs652.groundSeeds.winteredStems.length; wi++) {
      const ws = gs652.groundSeeds.winteredStems[wi];
      // Find the matching sprout
      const sprout = gs652.groundSeeds.sprouts ? gs652.groundSeeds.sprouts.find(function(sp) {
        return sp.group === ws.group;
      }) : null;
      if (sprout && sprout.leaves && sprout.leaves.length > 0) {
        // During autumn/winter, leaves should have been shed
        if (currentSeason === 'Winter' || currentSeason === 'Autumn') {
          problems.push('Wintered stem #' + wi + ' still has ' + sprout.leaves.length + ' leaves attached during ' + currentSeason + ' — leaves should have been shed (issue #652).');
        }
      }
    }
  } else if (gs652 && gs652.groundSeeds && gs652.seasonProgress !== undefined) {
    // No winteredStems — check if the season is one where they SHOULD be present
    // This is a soft check: only report if we're clearly in winter
    const seasonEl = document.getElementById('season-display');
    const currentSeason = seasonEl ? seasonEl.textContent.trim() : '';

    if (currentSeason === 'Winter' && gs652.groundSeeds.sprouts && gs652.groundSeeds.sprouts.length > 0) {
      // During winter, ANY sprout in the sprouts array with leafGenCount >= 1
      // should have been converted to a wintered stem. Check if any qualify.
      const qualifiedSprouts = gs652.groundSeeds.sprouts.filter(function(sp) { return sp.leafGenCount >= 1; });
      if (qualifiedSprouts.length > 0) {
        problems.push('During Winter, ' + qualifiedSprouts.length + ' sprouts have leafGenCount >= 1 but no winteredStems array exists — they should have been converted to bare stems (issue #652).');
      }
    }
  }
