...
18. **Garden Polish & Refinement** — Address the subtle performance and animation quality issues that emerge from a fully-featured garden: ensure the bloom animation always plays in correct order (bud → opens → flower eases in) without snapping or spinning, even under fast growth conditions; lazy-render closed panels (gallery, history, creature encyclopedia) so they cost nothing until opened; optimize initial load time by reducing the number of separate script and style files; and eliminate brief stutters when multiple flowers bloom simultaneously. This phase is not about new features — it is about honoring the 'Effortless Performance' and 'Organic Motion' principles at scale, ensuring the garden feels as calm and alive with 17 systems running as it did with one.


19. **Garden Rituals** — Small recurring moments that create rhythm in the gardener's relationship with their garden: a morning greeting that notes overnight changes and whispers a seasonal poem, a weekly tending summary that reflects on growth patterns and creature visits, and a gentle seasonal ceremony when the garden transitions between meteorological seasons. Rituals are not notifications or tasks — they are quiet invitations to pause and notice. They build on existing systems (whispers, journal, milestones, aging, seasons) and appear only when the gardener is already in the garden, never demanding attention. Each ritual should feel like the garden itself is initiating the conversation — not a feature reminding you to engage.
