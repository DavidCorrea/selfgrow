/**
 * The selfgrow interpreter core.
 * Provides createInterpreter() which returns a scoped evaluator with define/evaluate/run API.
 * Capability functions are registered via define() and are available as identifiers during evaluation.
 */
...
function makeInitialEnv(definitions) {
  const env = {};
...
function evaluate(ast, env, steps) {
  ...
  switch (ast.type) {
    ...
    case 'Call': {
      ...
      // Built-in and capability-defined functions
...
/**
 * Create a scoped interpreter where capability functions are available
 * as first-class identifiers. Capabilities register via define() before
 * evaluation runs.
 */
export function createInterpreter() {
  const definitions = {};

  function define(name, fn, arity = -1) {
    definitions[name] = {
      __builtin: true,
      fn: (args, steps) => fn(args),
      arity,
    };
  }

  // Boot: register the core print builtin
  define('print', (args) => {
  ...
  function makeEnv() {
    return makeInitialEnv(definitions);
  }
