...
function registerString(interpreter) {
  ...
  interpreter.addOperator('++', { precedence: 10, associativity: 'left', fn: (a, b, steps) => {
    if (typeof a !== 'string') {
      throw new TypeError(
...
export function checkProperties(run) {
  ...
  // Undefined symbol errors should be structured with location
  try {
    run('unknownSymbol');
    failures.push('unknownSymbol should throw an error');
  } catch (err) {
    if (!(err instanceof RuntimeError) && !(err instanceof TypeError)) {
      failures.push(`unknownSymbol should throw a SelfgrowError, got ${err.constructor.name}`);
    }
    if (!err.message.includes('unknownSymbol')) {
      failures.push(`undefined symbol error should mention the symbol name, got: ${err.message}`);
    }
    if (!err.location) {
      failures.push('undefined symbol error should include a location');
    }
    if (err.location && (!err.location.line || !err.location.column || err.location.offset === undefined)) {
      failures.push('undefined symbol error location should have line, column, and offset');
    }
    // Expected and found
    if (err.expected !== 'a defined symbol') {
      failures.push(`undefined symbol error expected should be 'a defined symbol', got: ${JSON.stringify(err.expected)}`);
    }
    if (err.found !== `symbol 'unknownSymbol'`) {
      failures.push(`undefined symbol error found should be "symbol 'unknownSymbol'", got: ${JSON.stringify(err.found)}`);
    }
  }
