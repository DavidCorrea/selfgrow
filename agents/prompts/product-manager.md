...
- **Defects** — measured layout faults: elements past the viewport edge, overlapping text, contrast below the WCAG minimum, containers collapsed to zero size, broken images, missing stylesheets, content squeezed into a narrow column of a desktop window, controls too small to tap on a phone. These are **reliable**: each names the element and the viewport it happens at. Ticket them.
